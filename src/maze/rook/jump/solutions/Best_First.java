...
public class Best_First {
	...
	public void callBestFirst(String location) {
		int n = 0;
		int i = 0, j = 0;
		...
		try {
			...
			while (!"G".equalsIgnoreCase(matrix[i][j])) {
				...
				if (i-jump>=0 && !visited.contains((i-jump)+","+j)) {
					...
					if (!path.containsValue((i-jump)+","+j)) {
						...
						tempH = bf.heuristic(heuristicArr, i-jump, j, n, matrix, heuristicElements);	
					} else {
						tempH = heuristicArr.get((i-jump)+","+j);
					}
					if (tempH != Integer.MAX_VALUE && tempH < minH) {
						minH = tempH;
						near=(i-jump)+","+j;
					} 
				}
				...
				if (j-jump>=0 && !visited.contains(i+","+(j-jump))) {
					...
					if (!path.containsValue(i+","+(j-jump))) {
						...
						tempH = bf.heuristic(heuristicArr, i, j-jump, n, matrix, heuristicElements);
					} else {
			...
			System.out.println("Shortest Path for BestFirst:");
